 ...
 *
 * cancel() used to only ever target NAME_LOCK - there was no way to cancel a stale NAME_UNLOCK
 * job at all, so a service restart that found the device newly LOCKED correctly enqueued a fresh
 * lock-side job but left any already-pending unlock-side job to run to completion regardless,
 * re-enabling features on a phone the user had since locked. Parameterised so
 * PrivacyMonitorService's restart catch-up can cancel the opposite direction too, symmetric to
 * the lock side.
 */
object PendingLockWork {
    // cancel() below would otherwise hand-write every log call twice (Log.X, then the identical
    // message again to DebugLogHelper.X), 6 pairs in one small function - the same dual-write
    // pattern PrivacyActionWorker/PrivacyMonitorService already collapse into their own
    // logDebug/logWarning/logError helpers. One shared pair here does the same for this object's
    // own single caller-supplied tag, so a message can only ever drift between the two sinks if
    // this one place gets it wrong, not up to 6 places.
    ...
    fun cancel(context: Context, tag: String, workName: String) {
        try {
            ...
            // cancelUniqueWork()'s returned Operation would otherwise be discarded - it resolves
            // asynchronously, and a genuine failure (WorkManager's own docs name a full internal
            // database as one real cause) could never be detected or logged, leaving a stale
            // lock/disable action free to still fire later with no error shown anywhere. Callers
            // here (broadcast receivers, an accessibility event handler) are not coroutines, so
            // this listens for the real outcome asynchronously rather than blocking to await it -
            // fire-and-forget for the caller, but the actual result is no longer silently
            // dropped. Runs the listener inline (Runnable::run): logging is thread-safe and
            // cheap, no need to hop threads.
